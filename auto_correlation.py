...
def auto_DD_jk(d1, d1_id, boxsize, gridsize, minsep, maxsep, nbins, nthreads):
        ...
        for a in [-1, 0, 1]:
            ...
            for b in [-1, 0, 1]:
                ...
                for c in [-1, 0, 1]:
                    ...
                    # Condition for autocorrelation
                    if s1 > s2:
                        ddpairs[s1][s2].append(ddpairs[s2][s1][0])
                        continue
                    ...
                    if np.size(xd1) != 0 and np.size(xd2) != 0:
                        ...
                        #Data pair counting
                        autocorr = 0
                        DD_counts = Corrfunc.theory.DD(autocorr, nthreads, bins, xd1, yd1, zd1, X2=xd2, Y2=yd2, Z2=zd2,
...
def auto_DD_to_tpcf_jk(d1, d1_id, boxsize, gridsize, minsep, maxsep, nbins, ddpairs):
    # Set up bins
    bins = 10. ** np.linspace(np.log10(minsep), np.log10(maxsep), nbins + 1)
    ...
    d2tot = np.size(d1, 0)  # Number of objects in d2
        ...
        for a in [-1, 0, 1]:
            ...
            for b in [-1, 0, 1]:
                ...
                for c in [-1, 0, 1]:
                    ...
                    dd_pairs_i[s1] = dd_pairs_i[s1] - ddpairs[s1][s2][0]

                    # Sum pairs for xi over entire sample
                    dd_pairs = dd_pairs + ddpairs[s1][s2][0]
    ...
    for s1 in range(N):
        ...
        # d1tot_s1 = np.size(d1, 0)
        # n1 = float(d1tot_s1) / Vbox
        # xi_i[s1] = dd_pairs_i[s1] / (n1 * n2 * (Vbox - gridsize**3) * Vshell) - 1
...
def auto_tpcf_jk(d1, boxsize, gridsize, minsep, maxsep, nbins, nthreads, jk_estimates=False):
    # Partition boxes
    print('Partition.')
    ...
    ddpairs = auto_DD_jk(d1, d1_id, boxsize, gridsize, minsep, maxsep, nbins, nthreads)

    # Compute xi
    print('Compute xi_hm.')
    xi, xi_i, meanxi_i, cov = auto_DD_to_tpcf_jk(d1, d1_id, boxsize, gridsize, minsep, maxsep, nbins, ddpairs)
